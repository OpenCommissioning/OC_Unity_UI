...
namespace Battlehub.RTHandles
{
    ...
    public class BoxSelection : RTEComponent, IBoxSelection
    {
        ...
        [SerializeField]
        public UnityEvent<GameObject> OnBoxSelection;
        ...
        private void HitTest()
        {
            ...
            var exposes = FindObjectsOfType<ExposeToEditor>();
            ...
            {
                Renderer r = exposes[i].GetComponentInChildren<Renderer>();
                Bounds bounds = r.bounds;
                GameObject go = exposes[i].gameObject;
            ...
            //{
            //    IEnumerable<Renderer> selectedRenderers = m_selectionRenderer.PixelPerfectDepthTest(renderers.ToArray(), SelectionBounds);
            ...
            // var halfExtents = selectionBounds.size / 2;
            // var forward = Camera.main.transform.forward;
            // var orientation = Quaternion.LookRotation(forward);
            // RaycastHit[] hits = Physics.BoxCastAll(Camera.main.transform.position, halfExtents, forward, orientation);
            //
            // foreach (var hit in hits)
            // {
            //     if (!hit.transform.TryGetComponent(out ExposeToEditor item)) continue;
            //     if (!item.CanTransform) continue;
            //     if (!selection.Contains(item.gameObject)) selection.Add(item.gameObject);
            // }
        ...
        private void FilterGameObjects(HashSet<GameObject> selection, FilteringArgs args, GameObject go)
        {
            if (!selection.Contains(go))
            {
                if (Filtering != null)
                {
                    ...
                    if (!args.Cancel)
                    {
                        //if (!(RuntimeSceneComponent.Instance.HoverLayerMask == (RuntimeSceneComponent.Instance.HoverLayerMask | (1 << go.layer)))) return;
                        if (!(((1 << go.layer) & RuntimeSceneComponent.Instance.HoverLayerMask.value) > 0)) return;
                        //if (!PlanX.Utils.IsLayerInSingleLayermask(go.layer, RuntimeSceneComponent.Instance.HoverLayerMask)) return;
                        selection.Add(go);
                        OnBoxSelection?.Invoke(go);
